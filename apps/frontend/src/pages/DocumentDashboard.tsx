...
import { listDocuments, deleteDocument, type DocumentSummary, type DashboardStats } from "../lib/api";
...
export function DocumentDashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({ total: 0, completedMonth: 0, pending: 0 });
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-7 p-10">
        ...
        <div className="grid grid-cols-3 gap-5" role="list" aria-label="Document statistics">
          ...
          <div className="bg-white rounded-lg border border-[#E5E7EB] p-6 flex flex-col gap-2" role="listitem">
            <span className="text-sm text-muted">{t("dashboard.completedMonth")}</span>
            <span className="font-display font-semibold text-[32px] text-success tracking-tight">
              {isLoading ? "—" : stats.completedMonth}
            </span>
          </div>
          <div className="bg-white rounded-lg border border-[#E5E7EB] p-6 flex flex-col gap-2" role="listitem">
            <span className="text-sm text-muted">{t("dashboard.pendingReview")}</span>
            <span className="font-display font-semibold text-[32px] text-amber tracking-tight">
                    ...
                    return (
                      <tr key={doc.id} className="border-b border-[#E5E7EB] last:border-0 hover:bg-[#FAFAFA] transition-colors">
                        ...
                        <td className="px-3 h-13">
                          <span className="text-sm text-text">{t(docTypeKey)}</span>
                        </td>
                        <td className="px-3 h-13">
                    ...
                    <ul className="flex items-center gap-1">
                      <li>
                        <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                          className="px-3 py-1.5 rounded-lg text-sm font-medium bg-white border border-[#E5E7EB] text-text hover:bg-gray-50 disabled:opacity-40" aria-label={t("pagination.previous")}>‹</button>
                      </li>
                      ...
                      <li>
                        <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                          className="px-3 py-1.5 rounded-lg text-sm font-medium bg-white border border-[#E5E7EB] text-text hover:bg-gray-50 disabled:opacity-40" aria-label={t("pagination.next")}>›</button>
                      </li>
                    </ul>
                  </nav>
                )}
